...
3. **QQ**：
   ...
   - 安装后建议禁用**开机启动**。
4. **Firefox**:
   - 下载地址： [https://download-installer.cdn.mozilla.net/pub/firefox/releases/141.0/win64/zh-CN/Firefox%20Setup%20141.0.exe](https://download-installer.cdn.mozilla.net/pub/firefox/releases/141.0/win64/zh-CN/Firefox%20Setup%141.0.exe)
   - 安装Firefox。
## 2. 进行配置
### 2.1 安装QQ
...
3. 根据文件内的指引进行配置：
   - 设置机器人QQ号（`SELF_ID`）
   - 配置远程API服务地址和密钥（`REMOTE_API_URL`, `REMOTE_API_KEY`）
   - 配置大模型API密钥
   - 设置其他功能选项（OCR、语音转文字等）
...
1. **基础配置** (`configs/base.yaml`)
   - 机器人QQ号 (`SELF_ID`)
   - 远程API服务配置（`REMOTE_API_URL`, `REMOTE_API_KEY`）
   - 大模型API密钥配置
   - 功能开关（OCR、语音转文字等）
...
### 3.3 功能依赖说明
- **OCR功能**：需要配置远程API服务（支持OCR功能）
- **语音转文字**：需要配置远程API服务（支持语音识别功能）
- **图片生成**：需要配置对应的大模型API
...
### 4.2 启动主程序
在项目目录中打开命令提示符（CMD）或 PowerShell，运行以下命令：
```bash
python main.py
```